...
test('validateRuntimeConfig usa el remitente por defecto si MAIL_FROM no esta definido', () => {
  const snapshot = { ...process.env };
  ...
  const { MAIL_FROM, validateRuntimeConfig } = loadEnvModule();

  assert.equal(MAIL_FROM, 'noreply@mariovillalonga.website');
  assert.doesNotThrow(() => validateRuntimeConfig());
  restoreEnv(snapshot);
});
